...
def main():
    with open("./scripts/K_plot_hedgehog.json", "r") as f:
    ...
    for i in range(NUM_RAND_REAL_TRIAL):
        df = pd.read_json(
            FOLDEROOT
            + f"Nothing/bs_{BUFF_SIZE}_real_random/real_random_{i:03}/metrics_growth.json"
        )
    df_concat = pd.concat([df_concat, df])
    real_random_df = pd.DataFrame(average_x(df_concat))
    dict_real_random_normalized = {}
    ...
    for met in MET_PLOT.keys():
        val_real_random_normalized = [0]
        for idx, target_x in enumerate(real_random_df["xx"][1:-1]):
            if target_x in random_df["xx"]:
                val_real_random_normalized.append(
                    random_df[met][list(random_df["xx"].index(target_x))]
                )
            else:
                abo_x = random_df["xx"][random_df["xx"] > target_x].min()
                ...
                bel_v = random_df[met][list(random_df["xx"]).index(bel_x)]
                val_real_random_normalized.append(
                    real_random_df[met][idx + 1]
                    - (bel_v + (target_x - bel_x) * (abo_v - bel_v) / (abo_x - bel_x))
                )
        val_real_random_normalized.append(0)
        dict_real_random_normalized[met] = val_real_random_normalized
    for ids, met in enumerate(plot_params["order"]):
        for normalized in [True, False]:
            for met_plot, met_label in MET_PLOT.items():
                ...
                if normalized:
                    ax.plot(
                        real_random_df["xx"] / 10**3,
                        np.array(dict_real_random_normalized[met_plot]) / ratio,
                        linewidth=LINEWIDTH,
                        zorder=2,
                        label="Real, interpolation",
                        color="pink",
                    )
                else:
                    ax.plot(
                        real_random_df["xx"] / 10**3,
                        real_random_df[met_plot] / ratio,
                        linewidth=LINEWIDTH,
                        zorder=2,
                        label="Real, interpolation",
                        color="pink",
                ...
                # Add growth
                for idx, (end_folder, pos) in enumerate(zip(END_FOLDERS, [0, 1, -2])):
                    df = pd.read_json(
                        FOLDEROOT
                        + end_folder
                    ...
                    if normalized:
                        ...
                        ax.plot(
                            ...
                            zorder=2,
                            alpha=1,
                            label=plot_params["label"][ids] + f", {end_folder}",
                            **{
                                key: val[ids]
                                for key, val in plot_params.items()
                                if key
                                not in ["dpi", "figsize", "rcparams", "order", "label"]
                            },
                        )
                    else:
                        ax.plot(
                ...
                fig.tight_layout()
                ax.legend()
                filename = folder_plot + f"/{met}_{met_plot}"
                if normalized:
